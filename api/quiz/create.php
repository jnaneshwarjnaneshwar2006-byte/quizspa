...
if (empty($title)) {
    sendJsonResponse(false, 'Quiz title is required.', [], 400);
}
...
try {
    ...
    // Insert Quiz
    $stmt = $pdo->prepare("INSERT INTO `quizzes` (`teacher_id`, `title`, `description`, `category`, `status`, `created_at`) VALUES (:teacher_id, :title, :description, :category, 'draft', NOW())");
    $stmt->execute([
    ...
    sendJsonResponse(true, 'Quiz created successfully!', ['quiz_id' => $quizId]);

} catch (Exception $e) {
    if (isset($pdo) && $pdo->inTransaction()) {
        $pdo->rollBack();
    }
    sendJsonResponse(false, 'Failed to create quiz: ' . $e->getMessage(), [], 500);
}
